...
| Story Service | 8082 | 소설, 카테고리, 문장 관리, **WebSocket 실시간 통계** |
...
| WebSocket (실시간 통계) | 4 | 4 | 0 | Unit Test |
| **합계** | **35** | **35** | **0** | **100%** |
...
### 4.4 WebSocket 실시간 통계 업데이트 테스트

**WebSocket 토픽 구독**
| 토픽 | 설명 | 발생 시점 |
|------|------|----------|
| `/topic/books/new` | 새 소설 생성 알림 | 소설 생성 시 |
| `/topic/books/stats` | 소설 통계 실시간 업데이트 | 문장 추가/삭제 시 |
| `/topic/sentences/{bookId}` | 특정 소설 문장 추가 알림 | 문장 이어쓰기 시 |

**HomeView 실시간 통계 업데이트 시나리오**
| 테스트 코드 | 분류 | | | 테스트 시나리오 | 테스트 방식 | 테스트 성공여부 |
|------------|------|---|---|----------------|------------|----------------|
| | 대분류 | 중분류 | 소분류 | | | (P/F) |
| WS-001.001.001 | WebSocket | Stats | 문장 추가 | 문장 이어쓰기 시 통계 브로드캐스트 | Unit Test | P |
| WS-001.001.002 | WebSocket | Stats | 문장 삭제 | 문장 삭제 시 통계 브로드캐스트 | Unit Test | P |
| WS-001.001.003 | WebSocket | Stats | 통계 없음 | 통계 조회 실패 시 메시지 미전송 | Unit Test | P |
| WS-001.002.001 | WebSocket | New Book | 소설 생성 | 새 소설 생성 시 브로드캐스트 | Unit Test | P |

**브로드캐스트 DTO 구조 (BookStatsUpdateDto)**
```json
{
  "bookId": 1,
  "sentenceCount": 5,
...
### 5.1 테스트 커버리지
```
전체 API 커버리지: 35/35 (100%)
- REST API: 31/31
- WebSocket 실시간 통계: 4/4
```
